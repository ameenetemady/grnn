...
do
  ...
  function  dataLoad.loadInput()
    local nInputCols = settings.cascadeA.nInputCols
    local teInput = dataLoad.loadFromTsv(settings.cascadeA.inputFilename, nInputCols)
    return teInput
  end

  function dataLoad.loadTarget()
    local nTargetCols = settings.cascadeA.nTargetCols
    local teTarget = dataLoad.loadFromTsv(settings.cascadeA.targetFilename, nTargetCols)
    return teTarget
